...
public class ZhipuProvider implements AiProvider {
    ...
    public ZhipuProvider(
            @Value("${ai.providers.zhipu.api-key}") String apiKey,
            @Value("${ai.providers.zhipu.base-url}") String baseUrl,
            @Value("${ai.providers.zhipu.model}") String model,
            @Value("${ai.providers.zhipu.max-tokens}") int maxTokens) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    @Override
    public String getProviderName() { return "zhipu"; }

    @Override
    public String getModelName() { return model; }

    @Override
    public boolean isAvailable() { return !apiKey.equals("your-zhipu-api-key"); }
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        try {
            ...
            log.error("智谱GLM API 调用失败: {}", e.getMessage());
        }
        throw new RuntimeException("智谱GLM API 调用失败");
    }
    ...
    public List<EmbeddingResponse> embed(List<String> texts) {
        throw new UnsupportedOperationException("智谱GLM 暂不支持 Embedding API");
    }
}
