...
public class MimoProvider implements AiProvider {
    ...
    public MimoProvider(
            @Value("${ai.providers.mimo.api-key}") String apiKey,
            @Value("${ai.providers.mimo.base-url}") String baseUrl,
            @Value("${ai.providers.mimo.model}") String model,
            @Value("${ai.providers.mimo.max-tokens}") int maxTokens) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    @Override
    public String getProviderName() { return "mimo"; }

    @Override
    public String getModelName() { return model; }

    @Override
    public boolean isAvailable() { return !apiKey.equals("your-mimo-api-key"); }
    ...
    public ChatResponse chat(ChatRequest request) {
        ...
        try {
            ...
            log.error("小米MiMo API 调用失败: {}", e.getMessage());
        }
        throw new RuntimeException("小米MiMo API 调用失败");
    }
    ...
    public List<EmbeddingResponse> embed(List<String> texts) {
        throw new UnsupportedOperationException("小米MiMo 暂不支持 Embedding API");
    }
}
