...
public class DeepSeekProvider implements AiProvider {
    ...
    public DeepSeekProvider(
            ...
            @Value("${ai.providers.deepseek.base-url}") String baseUrl,
            @Value("${ai.providers.deepseek.model}") String model,
            @Value("${ai.providers.deepseek.max-tokens}") int maxTokens) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    @Override
    public boolean isAvailable() { return !apiKey.equals("your-deepseek-api-key"); }
    ...
    public List<EmbeddingResponse> embed(List<String> texts) {
        throw new UnsupportedOperationException("DeepSeek 暂不支持 Embedding API");
    }
}
